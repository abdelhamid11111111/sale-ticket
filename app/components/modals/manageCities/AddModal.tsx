...
const AddModal = ({ onAddCity }: onAddCityProp) => {
  ...
  return (
    <div>
      <button
        onClick={handleModal}
        className="inline-flex cursor-pointer items-center px-4 py-2 text-sm font-medium text-white bg-[#135bec] rounded-lg hover:bg-[#0e4ac4] shadow-sm transition-colors"
      >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-lg p-6 shadow-lg w-full max-w-md">
            {/* Header */}
            <div className="mb-4">
              <h2 className="text-lg font-bold text-[#111418]">Add City</h2>
            </div>
